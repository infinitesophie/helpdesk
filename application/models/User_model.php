...
class User_Model extends CI_Model 
{
    ...
    public function get_usernames_user_role($username) 
    {
    	return $this->db
    		->like("users.username", $username)
    		->join("user_roles", "user_roles.ID = users.user_role")
    		->where("user_roles.admin", 1)
    		->or_where("user_roles.admin_members", 1)
    		->or_where("user_roles.ticket_manager", 1)
    		->or_where("user_roles.ticket_worker", 1)
    		->limit(10)
    		->get("users");
    }
    ...
    public function get_notifications($userid) 
    {
    	return $this->db
    		...
    		->join("users", "users.ID = user_notifications.fromid", "left outer")
    		->limit(5)
    		->order_By("user_notifications.ID", "DESC")
    		->get("user_notifications");
    }

    public function get_all_unread_noti($userid) 
    {
    	return $this->db
    		->where("userid", $userid)
    		->where("status", 0)
    ...
    public function add_notification($data) 
    {
    	if(isset($data['email']) && isset($data['email_notification']) 
	    	...
	    	// Send Email
	    	$subject = $this->settings->info->site_name . " " . lang("ctn_500");
    ...
    {
    	return $this->db->where("status", 1)->get("announcements");
    }

    public function get_announcement($id) 
    {
    	return $this->db->where("ID", $id)->get("announcements");
    }
