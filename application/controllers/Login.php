...
class Login extends CI_Controller 
{
	...
	public function ajax_check_login() 
	{
		...
		$field_errors = array();
		if(empty($data['email'])) {
			$field_errors['email'] = lang("error_131");
		}
		if(empty($data['pass'])) {
			$field_errors['pass'] = lang("error_132");
		}
	...
	{	
		...
		$role = $this->user_model->get_user_role($roleid);
		if($role->num_rows() > 0) {
			$role = $role->row();
			if($role->client) {
				$redirect = "client";
